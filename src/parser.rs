use crate::ast::*;
use crate::lexer::{AssignOp, Lexer, Op, TokenType};
...
fn sequence(lexer: &mut Lexer) -> Statement {
    ...
    loop {
        ...
        match token.token_type {
            ...
            TokenType::Eof => break,
            t => panic!(
                "bad token: {:?}, token number: {}, location: {}",
                t, lexer.token_num, token.pos
            ),
        }
    }
...
fn statement(lexer: &mut Lexer) -> Statement {
    ...
    match lhs.token_type {
        TokenType::Variable(ref var) => {
            let op = lexer.peek();
            match op.token_type {
                TokenType::Assign(_) => assign(var, lexer),
                TokenType::Operator(op) if op == Op::Inc || op == Op::Dec => {
                    op_statement(var, op, lexer)
                }
                _ => {
...
fn assign(var: &String, lexer: &mut Lexer) -> Statement {
    let token = lexer.next();
    let mut rhs = Box::new(expr(None, lexer));
    match token.token_type {
        TokenType::Assign(a) => {
            let var = Box::new(Expr::Variable(var.to_string()));
            rhs = match a {
                AssignOp::Number => rhs,
...
fn var_list(lexer: &mut Lexer) -> Vec<String> {
    ...
    loop {
        ...
        if let TokenType::Variable(v) = token.token_type {
            vars.push(v);
            token = lexer.next();
            ...
            lhs = match op {
                TokenType::Operator(op) => {
                    ...
                    match op {
                        ...
                        _ => panic!(
                            "bad op: {:?}, token number: {}, location: {}",
                            op, lexer.token_num, token.pos
                        ),
                    }
                }
                TokenType::Then => {
...
mod tests {
    ...
    fn sponge() {
        ...
        let tokens = crate::lexer::lex(&mut i_str);
        dbg!(&tokens);
        let s = parse(&mut i);
        dbg!(s);
    }
